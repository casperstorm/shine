/* eslint-disable react/display-name, no-unused-vars, react/prop-types */
/* eslint-disable standard/no-callback-literal, import/no-unresolved, import/no-extraneous-dependencies */
/* eslint-env jest */
/* eslint-env node */

// ----------------------------------------------------------------------------------------
// Content copied from
// https://github.com/facebook/react-native/commit/7fb3a9229df52bd45076470d059f245a8147cd2a#diff-606adbd6a8c97d177b17baee5a69cdd9
// ----------------------------------------------------------------------------------------

global.__DEV__ = true

const mockComponent = moduleName => {
      ...
      function ListViewDataSource(dataBlob) {
        ...
        try {
          Object.keys(dataBlob).forEach(key => {
            this.items +=
              dataBlob[key] && (dataBlob[key].length || dataBlob[key].size || 0)
          })
        } catch (e) {
          this.items = 'unknown'
        }
    ...
    ;[
      ...
      NativeMethodsMixin[key] = function() {
        ...
        console.warn(
          'Calling .' +
            key +
            '() in the test renderer environment is not ' +
